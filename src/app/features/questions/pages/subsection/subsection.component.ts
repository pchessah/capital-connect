...
export class SubSectionComponent{
  ...
  questions$ = this._activatedRoute.params .pipe(tap((res) =>{
    // @ts-ignore
    this.routeId =res['id'].trim();
    const ids =this.routeId.split('-');
    this.sectionId =Number(ids.at(0))
    this.subSectionId =Number(ids.at(1));
    this.sectionInfo$ =this._questionsService.getSingleSubsection(this.subSectionId).pipe(tap(subsection =>{
      this.subsectionName =subsection.name;
    }))

    this.questionsFetch$ =this._questionsService.getQuestionsOfSubSection(this.subSectionId).pipe(tap(vals => {
      this.questions =vals;
    }))
  }))
